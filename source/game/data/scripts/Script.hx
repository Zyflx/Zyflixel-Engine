...
class Script
{
	...
	public function new(scriptName:String)
	{
		...
		try
		{
			if (scriptFile != null)
			{
				...
				set('Preferences',
					game.state.menus.options.PreferencesMenu); // just in case you ever want to check if downscroll if on or somethin in a script
	...
	public static function runScripts(scriptArr:Array<Script>, folders:Array<String>)
	{
		...
		if (scriptArr != null)
			for (scripts in scriptArr)
			{
				scripts.callFunction('create');
			}
		return scriptArr;
	}
...
class Script
{
	...
	public function set(var:String, val:Dynamic)
	{
		return null;
	}
